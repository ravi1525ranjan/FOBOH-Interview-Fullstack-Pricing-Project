...
export default function PricePreview({ products, profileName }: Props) {
  ...
  // Filter profiles to show only those containing the selected products
  ...
  //  Determine Base Price from selection or Global
  const getBasePrice = (product: Product) => {
    ...
    return savedItem
      ? savedItem.adjustment
      : (product.globalWholesalePrice ?? 0);
  };
  ...
  // Debounce input for API efficiency
  useEffect(() => {
  ...
  //CORE CALCULATION: Sync with Preview API
  useEffect(() => {
    ...
    const rows: PricingRowPayload[] = products.map((p) => ({
      productId: p.id,
      adjustment:
        typeof rowState[p.id]?.adjustment === "number"
          ? (rowState[p.id]?.adjustment as number)
          : 0,
    }));
      ...
      setPriceMap((prev) => {
        ...
        res.forEach((r) => {
          ...
          const userAdjustment =
            typeof rowState[r.productId]?.adjustment === "number"
              ? (rowState[r.productId]?.adjustment as number)
              : 0;
  ...
  const handleSaveProfile = async () => {
    ...
    const payload: any = {
      name: profileName,
      basedOnProfileId:
        selectedProfileId === "global" ? null : selectedProfileId,
