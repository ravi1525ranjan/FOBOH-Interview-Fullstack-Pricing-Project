...
export default function Sidenav({ isCollapsed }: SidenavProps) {
  ...
  return (
    ...
    >
      {/* Brand Section */}
      <div
        className="d-flex align-items-center mb-4 text-white text-decoration-none justify-content-center"
        style={{ height: "40px" }}
      >
        {!isCollapsed ? (
            ...
            className={({ isActive }) =>
              `nav-link d-flex align-items-center ${isCollapsed ? "justify-content-center" : "gap-3"} ${
                isActive
                  ...
                  : "text-white opacity-75 nav-hover"
              }`
            }
            style={{ transition: "all 0.2s ease" }}
          >
            {/* UPDATED PATH: Direct from public folder */}
            <img
              src={`/${item.icon}.svg`}
              width="20"
              height="20"
            ...
            {!isCollapsed && (
              <span className="fw-medium text-nowrap">{item.label}</span>
            )}
          </NavLink>
        ))}
      </nav>
